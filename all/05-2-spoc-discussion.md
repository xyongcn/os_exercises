...
class scheduler:
    ...
    #change to WAITING STATE, the current proc's state should be expected
    ...
    def next_proc(self, pid=-1):
        if pid == -1:
            pid = self.curr_proc
        for i in range(pid+1,num_instructions):
