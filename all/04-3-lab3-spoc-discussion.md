...
```
代码和FIFO基本相同。只是要额外维护一个链表保存访问bit，在swap_out_victim和map_swappable的时候，
用一个指针找到第一个访问bit数值最小的页，对它操作的同时修改指针经过的页的访问bit。

当某个页被访问很多次而其他页只被访问很少的时候（例如access pattern为abcddddefgdddd...的时候），
由于clock算法只记录页是否出现过，访问序列经过恰当的排列之后，性能会比LRU明显差很多。
